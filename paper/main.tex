...
\begin{titlepage}
    ...
    \noindent
    \large
    Project Code: \texttt{\ProjectCode} \\
    Name: \StudentName \\
    CID: \CID \\
...
\vspace{-0.3cm}
\begin{center}
{\normalsize\bfseries When Virtual Galaxies Shine Too Bright}\\[2pt]
{\small Mihir Koka}
\end{center}
\begin{spacing}{1.0}
...
I first checked that the model reproduces observed brightnesses close to the
  ...
  longer wavelengths would therefore point to an issue with extrapolating the model rather than a
  general issue in the catalogue.
...
% --- 5. What the modification showed ---
...
extrapolation against Herschel/SPIRE observations at 250, 350 and
500{\um}. Nine published differential-count products
...
$[+0.16,+0.52]$. This is used as a way to test robustness rather than a proper confidence
interval.
...
0.20 and \SI{0.15}{dex} at 350 and \SI{500}{\um}.
The main recommendation is to keep the learned population and energy balance,
but to add a galaxy-dependent FIR dust family and validate both population counts
...
infrared after starlight is absorbed and re-radiated by interstellar dust
...
is not simple. Population-level generative modelling offers an alternative. Rather than
fitting each galaxy separately, one can model the joint distribution of physical parameters
...
This makes \popcosmos{} useful for simulating surveys, but it also makes validation beyond the calibration regime important to make larger claims. A reasonable
catalogue in the fitted bands is not necessarily accurate at unseen regimes.
...
by  energy balance. The total luminosity absorbed by dust in the ultraviolet and
...
The initial comparison selected finite model values with
...
These tests provided useful context but could not properly determine whether the
...
Validating a population model in a new wavelength regime requires a comparison metric.
  ...
  obscured SFR against published catalogue values. However in reality this is awkward because catalogue
...
reconciliation is neede. Counts are a long-established diagnostic with a substantial
...
required resolving its own problems. Therefore, following a supervisor discussion it was decided it was recommended that the project's scope be narrowed to the far-infrared and that
validation focus on directly observed quantities. \\
...
\SI{10}{mJy}. Therefore the selection has little effect over the main
10--\SI{100}{mJy} comparison range.
...
(H-ATLAS, HerMES, Dark Field) cover independent sky areas and provide the main benchmark. The remaining tables overlap these fields and serve as
...
across a random sample of 500 galaxies is $1.00002$, confirming  consistency of
the stored normalisation and unit conversion.
...
174 bins from nine published products across all six studies. The smaller subset only uses
...
distribution, per-object scatter has an effect on number
...
The model reproduces the near-infrared photometry to within a few hundredths of a
  magnitude across the full redshift range. This supports the conclusion that the
...
failure but are not used as the main quantitative claim.
...
\subsection{Why raw Wang counts were not used as the benchmark}
\label{sec:results:wang_counts}

Early in the project, the Wang catalogue was also treated as a source-count
measurement. The first calculation divided the catalogue by a rough COSMOS area of
\SI{2.0}{\deg\squared}. Replacing this with the relevant Farmer-selected area of
\SI{1.278}{\deg\squared} raised the Wang surface density by a factor of 1.56 and
reduced the apparent disagreement, but did not remove it. This led to a separate check
against the Jin super-deblended catalogue and the published, corrected counts.

\begin{figure}[H]
\centering
% The top trim removes an older working title whose interpretation was superseded
% by the later flux-resolved analysis.
\makebox[\textwidth][c]{\includegraphics[width=1.15\textwidth,trim=0 0 0 35,clip]{Figures/wang_jin_count_diagnosis.png}}
\caption{Raw differential counts constructed from the Wang and Jin COSMOS catalogues,
compared with published corrected SPIRE counts. The dashed Wang curve applies only a
simple wavelength-dependent flux rescaling inferred from matched Wang--Jin sources. It
is not a completeness correction. This rescaling improves the agreement at lower and
intermediate flux, particularly at 350 and \SI{500}{\um}, while both catalogues still fall below the other field measurements above approximately \SI{30}{mJy}.}
\label{fig:wang_jin_counts}
\end{figure}

This result was not interpreted as evidence that the Wang fluxes
were generally wrong. Wang and Jin are prior-selected photometry catalogues from the
same small field, whereas the published count tables include survey-specific
completeness and flux-bias corrections. The mild long-wavelength flux bias reported
by \citet{Wang_2024} is amplified by the steep count slope, and the rare bright-source
regime is poorly sampled in a COSMOS-sized field. Raw Wang and Jin counts were
consequently retained as a diagnostic, while the formal population comparison used the
published differential counts. The older raw Wang overlays were superseded by this
test and are not used to support the headline count excess.
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=\textwidth]{Figures/drew_casey_peak_relation.png}}
...
dashed low-AGN fit to the exploratory model sample is nearly flat
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.15\textwidth]{Figures/agn_hot_dust_diagnosis.png}}
...
The excess does not come from one fixed redshift range. At 250~$\upmu$m, the
...
\begin{figure}[H]
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.15\textwidth]{Figures/lensing_bright_end_assessment.png}}
...
\citep{Wardlow_2013,Negrello_2017}. Right: the \SI{300}{mJy} count divided by the \SI{244}{mJy} count in
each band. The central ratios decline with wavelength, but the differences are smaller
than the uncertainties, so no wavelength trend can be made. The plotted
errors use the published per-bin uncertainties but do not include unknown bin covariance.}
...
\subsection{Confidence of results and alternative explanations}
\label{sec:results:robustness}

Five potential concerns were considered.
...
Removing any single survey family does not change the polarity of the excess.
...
10--\SI{100}{mJy}. Brighter bins are shown but do not support the main numerical claim.
...
The optical and near-infrared tests shows that the catalogue and flux pipeline work
...
The two findings therefore need their own changes to fix
...
chi-square score should not be interpreted as identifying the correct  model.
...
be accompanied by distributions of individual SED properties. The link
between the hot branch and $f_\mathrm{AGN}$ is reasonable looking at it from a physics view, but its frequency
cannot be interpreted as a measured AGN fraction. The fitted parameter has a
...
The Wang--Jin agreement provides a useful control test. The two deblending catalogues agree with
...
inference. These limitations mean the selection of a particular replacement model is weak but the main result that the baseline far-infrared handling needs improvement remains.
...
Rare nearby galaxies, small-number statistics, gravitational lensing and the count-inversion procedure may also contribute, but this work cannot tell them apart.
...
out a lensing contribution, just that these bins cannot identify one on their own.
...
250~$\upmu$m and none at 350 or 500~$\upmu$m. But regardless it should be
explicitly considered if validation is extended to a much wider field as initially intended. 


\subsection{Implications and recommended extension of \popcosmos{}}
\label{sec:discussion:recommendations}

The results suggest several ways forward for future \popcosmos{} work.

The most direct physical extension would be to replace the fixed far-infrared bump with a
...
Technical improvements could include integration through the true SPIRE bandpasses,
...
informs the required direction, warmer and luminosity-dependent, but does not
select a dust model on its own. The full catalogue also contains a distinct hot,
AGN-associated SED branch near \SI{14}{\um}. Unfortunately it is hidden by median stacks and
its population fraction is uncertain because the analysis uses posterior-median
...
confirmed in the published tables, but its origin was not found. Results above
...
available through CIGALE \citep{Boquien_2019} provide possible starting points. Future validation could calibrate
...
guidance, feedback and encouragement throughout this project. To Boris for providing the \popcosmos{} SED products, the supporting data used and help in the analysis, and to David for his advice and expertise on the far-infrared observations and the interpretation of the results. I also thank the \popcosmos{} collaboration and the
...
cross-checking numerical summaries, LaTeX formatting and drafting sentences before a final report was formally written.

% ===================== Appendix =====================
\appendix
\section*{Appendix}

The original pop-cosmos repository was used to understand the model and use of FSPS. Its notebooks provided a useful starting point for the analysis done in this thesis. The
original framework is available at
\href{https://github.com/Cosmo-Pop/pop-cosmos}
{github.com/Cosmo-Pop/pop-cosmos}.

The code and derived research products created for this thesis are available at
\href{https://github.com/IlKoKaIl/Pop-Cosmos-Thesis-Products}
{github.com/IlKoKaIl/Pop-Cosmos-Thesis-Products}. The repository contains the
Python analysis scripts used to generate the SED comparisons, observed-frame
flux predictions, differential number-count comparisons, catalogue diagnostics,
model-evaluation scores, and thesis figures. 

Some large input catalogues and model products are not included in the public
repository. These include the COSMOS2020 and Wang catalogues, the Jin et al.
super-deblended catalogue, and HDF5 products supplied (By Boris) for the pop-cosmos FSPS
SEDs and infrared luminosities. These files are either publicly available from
or were supplied to me for this project.
